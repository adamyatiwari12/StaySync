...
export default function RoomCard({
  ...
  const percentFull = Math.round((room.occupiedCount / room.capacity) * 100);
        ...
        <div className="space-y-3">
          ...
          <div className="mt-5 pt-4 border-t border-border">
            <div className="flex justify-between items-center mb-3">
              <h4 className="text-sm font-semibold">Assigned Residents</h4>
              {!isFull && (
                <button
                  onClick={() =>
                    router.push(`/admin/rooms/assign?roomId=${room._id}`)
                  }
            ...
            {room.tenants?.length ? (
              ...
              <div className="text-center py-4 bg-background-muted rounded-lg border border-dashed border-border">
                <p className="text-xs text-text-muted">No residents assigned</p>
                <button
                  onClick={() =>
                    router.push(`/admin/rooms/assign?roomId=${room._id}`)
                  }
